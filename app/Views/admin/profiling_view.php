...
<div class="main-container">
    ...
    <div class="card-header bg-primary bg-gradient text-white p-3 border-0">
        <div class="d-flex align-items-center justify-content-between">
            <h5 class="mb-0">
                <i class="fas fa-map-marker-alt me-2"></i> Kabupaten Sudah Isi Data
            </h5>
            <span id="jumlahKab" class="badge bg-white text-primary rounded-pill fs-6 px-3">
                <!-- Angka muncul di sini -->
                0
            </span>
        </div>
    </div>
    ...
    <div class="card-body p-4 bg-light">
        <div id="listKab" class="d-flex flex-wrap gap-2">
            <!-- Item List akan di-inject ke sini -->
            <!-- Contoh tampilan Item: 
            <span class="badge bg-white text-dark border p-2 px-3 shadow-sm">Kab. Malang</span> 
            -->
        </div>
    </div>
</div>
            ...
            <div class="card-header bg-white border-0 pt-4 px-4">
                <div class="d-flex align-items-center justify-content-between">
                    <h4 class="fw-bold text-dark mb-0">
                        <i class="fas fa-table text-primary me-2"></i> Analisis Data Tabular
                    </h4>
                </div>
            </div>

            <div class="card-body px-4">
                <!-- Tab Navigation Style Modern -->
                <ul class="nav nav-pills nav-fill mb-4 bg-light p-1 rounded-pill" id="tabDataTabular">
                    <li class="nav-item">
                        <button class="nav-link active rounded-pill fw-bold tab-btn btn-populasi" onclick="showTab('populasi', this)">
                            <i class="fas fa-chart-line me-1"></i> Populasi
                        </button>
                    </li>
                    <li class="nav-item">
                        <button class="nav-link rounded-pill fw-bold tab-btn btn-produksi" onclick="showTab('produksi', this)">
                            <i class="fas fa-boxes me-1"></i> Produksi
                        </button>
                    </li>
                    <li class="nav-item">
                        <button class="nav-link rounded-pill fw-bold tab-btn btn-harga" onclick="showTab('harga', this)">
                            <i class="fas fa-tag me-1"></i> Harga
                        </button>
                    </li>
                </ul>

                <!-- Table Containers dengan Responsive Wrapper -->
                <div class="table-responsive rounded-3 border">
                    <!-- Tab Populasi -->
                    <div id="tabPopulasi" class="tab-content-item">
                        <table class="table table-hover align-middle mb-0">
                            <thead class="table-primary text-nowrap" id="headPopulasi"></thead>
                            <tbody id="tablePopulasi" class="text-nowrap"></tbody>
                        </table>
                    </div>

                    <!-- Tab Produksi -->
                    <div id="tabProduksi" class="tab-content-item" style="display:none;">
                        <table class="table table-hover align-middle mb-0">
                            <thead class="table-success text-nowrap" id="headProduksi"></thead>
                            <tbody id="tableProduksi" class="text-nowrap"></tbody>
                        </table>
                    </div>

                    <!-- Tab Harga -->
                    <div id="tabHarga" class="tab-content-item" style="display:none;">
                        <table class="table table-hover align-middle mb-0">
                            <thead class="table-warning text-nowrap" id="headHarga"></thead>
                            <tbody id="tableHarga" class="text-nowrap"></tbody>
                        </table>
                    </div>
                </div>
                
                <div id="emptyState" class="text-center py-5 d-none">
                    <img src="https://cdn-icons-png.flaticon.com" width="80" class="opacity-50 mb-3">
                    <p class="text-muted">Pilih Provinsi & Kabupaten untuk melihat data</p>
                </div>
            </div>
        </div>

        <style>
            /* Bikin tab aktif jadi berwarna sesuai fungsinya */
            .nav-pills .nav-link.active.btn-populasi { background-color: #0d6efd !important; }
            .nav-pills .nav-link.active.btn-produksi { background-color: #198754 !important; }
            .nav-pills .nav-link.active.btn-harga { background-color: #ffc107 !important; color: #000 !important; }
            .nav-pills .nav-link { color: #6c757d; border: none; transition: 0.3s; }
            .table thead th { font-weight: 600; text-transform: uppercase; font-size: 0.85rem; letter-spacing: 0.5px; }
        </style>


        
        <!-- pilihan tahun utk ai analisis -->
...
<!-- ajax Data Tabular -->
<script>
    document.getElementById('provinsiDropdown').addEventListener('change', loadTableData);
document.getElementById('kabupatenDropdown').addEventListener('change', loadTableData);

function formatAngka(angka) {
    if (!angka || angka == '-' || angka == 'null') return '-';
...
function showTab(tab, el) {
    ...
    document.getElementById('tabPopulasi').style.display = 'none';
    document.getElementById('tabProduksi').style.display = 'none';
    document.getElementById('tabHarga').style.display = 'none';
...
function loadTableData() {
    ...
    let prov = document.getElementById('provinsiDropdown').value;
    let kab  = document.getElementById('kabupatenDropdown').value;

    if (!prov || !kab) return;

    fetch(`/admin/profiling/getTableData?provinsi=${prov}&kabupaten=${kab}`)
    .then(res => res.json())
    .then(data => {

        console.log("DATA:", data);

        renderPopulasi(data.populasi);
        renderProduksi(data.produksi);
        renderHargaPivot(data.harga);
    });
}

// render populasi
function renderPopulasi(data) {

    let grouped = {};

    data.forEach(row => {
        let jenis = row.jenis_ternak;

        if (!grouped[jenis]) grouped[jenis] = {};

        grouped[jenis][row.tahun] = row.jumlah_populasi;
    });

    let tahunList = [...new Set(data.map(d => d.tahun))];

    let html = `
        <tr>
            <th>Populasi</th>
            ${tahunList.map(t => `<th>${t}</th>`).join('')}
        </tr>
    `;
    ...
    for (let jenis in grouped) {
        ...
        tahunList.forEach(t => {
            html += `<td>${formatAngka(grouped[jenis][t])}</td>`;
        });

        html += `</tr>`;
    }

    document.getElementById('tablePopulasi').innerHTML = html;
}

// render produksi
function renderProduksi(data) {

    let grouped = {};

    data.forEach(row => {
        let jenis = row.jenis_produksi;

        if (!grouped[jenis]) grouped[jenis] = {};

        grouped[jenis][row.tahun] = row.jumlah;
    });
    ...
    document.getElementById('tableProduksi').innerHTML = html;
}

// render harga
function renderHargaPivot(data) {

    let grouped = {};

    data.forEach(row => {
        let kategori = row.kategori || 'Lainnya';
        let jenis = row.jenis;
        ...
        if (!grouped[kategori][jenis]) {
            grouped[kategori][jenis] = {};
        }

        grouped[kategori][jenis][row.tahun] = row.harga;
    });
    ...
    for (let kategori in grouped) {
        ...
        html += `
            <tr style="background:#f1f1f1;">
                <th colspan="${tahunList.length + 1}">${kategori}</th>
            </tr>
            <tr>
                <th>Jenis</th>
                ${tahunList.map(t => `<th>${t}</th>`).join('')}
        ...
        // 🔥 LOOP JENIS
        for (let jenis in grouped[kategori]) {
            html += `<tr><td>${jenis}</td>`;

            tahunList.forEach(t => {
                html += `<td>${formatRupiah(grouped[kategori][jenis][t])}</td>`;
            });
    ...
    document.getElementById('tableHarga').innerHTML = html;
}

function formatRupiah(angka) {
    if (!angka) return '-';

    angka = angka.toString().replace(/[^0-9]/g, ''); // bersihin Rp & koma

    return 'Rp ' + new Intl.NumberFormat('id-ID').format(angka);
}
</script>

<!-- js kab yg sudah mengisi -->
<script>
    function loadKabTerisi() {
    fetch('/admin/profiling/getKabupatenTerisi')
    .then(res => res.json())
    .then(data => {
        let jumlah = data.length;

        // Tampilkan jumlah dengan gaya Badge atau text Bold yang bersih
        document.getElementById('jumlahKab').innerHTML = `
        ...
        // Kita buat looping yang menghasilkan kotak-kotak kecil (badges)
        data.forEach(row => {
            html += `
                <div class="p-2 border rounded bg-white shadow-sm d-flex align-items-center" 
                     style="font-size: 0.9rem; min-width: 200px; border-left: 4px solid #0d6efd !important;">
                    <i class="fas fa-check-circle text-success me-2"></i>
                    <div>
                        <strong class="d-block text-dark">${row.kab_kota}</strong>
                        <small class="text-muted" style="font-size: 0.75rem;">${row.provinsi}</small>
                    </div>
                </div>`;
        });

        // Masukkan ke dalam container (pastikan gunakan d-flex flex-wrap di HTML-nya)
        document.getElementById('listKab').innerHTML = `
            <div class="d-flex flex-wrap gap-2 mt-3">
                ${html || '<p class="text-muted italic">Belum ada data tersedia.</p>'}
            </div>`;
    })
