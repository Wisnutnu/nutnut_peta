...
// ==========================
        ...
        return view('admin/profiling_view', $data);
    }

// tambaan data tabular untuk chart
public function getTableData()
{
    ...
    // POPULASI
    $pop = $db->table('populasi')
        ->where(['provinsi'=>$prov, 'kab_kota'=>$kab])
        ->get()->getResult();

    // PRODUKSI
    $prod = $db->table('produksi')
        ->where(['provinsi'=>$prov, 'kab_kota'=>$kab])
        ->get()->getResult();

    // HARGA (ambil konsumen saja biar rapi)
    $harga = $db->table('harga')
    ->select("
        kategori,
        jenis_ternak as jenis,
        tahun,
        REPLACE(harga, ',', '') as harga
    ")
    ->where([
        'provinsi'=>$prov,
        'kab_kota'=>$kab
...
// nyari kab yg sudah ngisi data
public function getKabupatenTerisi()
{
    $db = \Config\Database::connect();

    $result = $db->table('populasi')
        ->select('provinsi, kab_kota')
        ->groupBy('provinsi, kab_kota')
        ->get()
        ->getResult();
